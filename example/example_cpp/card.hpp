...
void get_data_from_json(json all_data)
{
    ...
    for(auto i=CARD_DICT.begin();i!=CARD_DICT.end();i++)
    {
        ...
        {
            i->second[level] = Creature(name, data["duplicate"], level, data["cost"][level-1],
    ...
    INFERNO = Creature("Inferno", creature_data["Inferno"]["duplicate"], 1, creature_data["Inferno"]["cost"][0],
