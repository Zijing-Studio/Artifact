...
1. 神迹剩余血量更多的一方获胜
2. 如果神迹剩余血量相同，计算所有阵亡的己方生物的 **星级** 之和，阵亡己方生物星级和较小的一方获胜
...
- **生成** ： (神迹范围5 或 占领驻扎点范围3) 且 无地面生物 且 非深渊 的位置
- **召唤物** ： 8/8/1-1/3
...
因此，只需判断， `ally.atk_range[0] <= dis <= ally.atk_range[1]` 是否为真，即可得知 `ally` 能否攻击神迹。
...
SALAMANDERSHIELD = Artifact("SalamanderShield", 6, 6, "Unit") # 阳炎之盾
